...
一款面向 Songloft 的 JS 插件，可以实现按文件夹视图的方式浏览歌曲库，支持在双栏界面中完成浏览、播放和整理。
...
- 单独安装：下载仓库 `releases` 目录中的 `folder-browser-v0.4.7.jsplugin.zip`
...
- 歌曲操作改为列表右侧固定图标按钮，移除三点弹出菜单
- 新增红心收藏与取消收藏，状态与 Songloft 内置“收藏”歌单同步
- 操作列横向滚动时固定在右侧，不再随自定义属性列移出视图
- 缩小歌手、格式、专辑、时长、年份、流派、码率等属性列的默认宽度
- 修复时长列未左对齐的问题，并完成移动端图标操作区适配

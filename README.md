...
- 单独安装：下载仓库 `releases` 目录中的 `folder-browser-v0.4.5.jsplugin.zip`
...
- 新增“加入播放队列”批量操作，不创建或修改歌单
- 当前没有播放队列时自动创建队列并从第一首开始播放
- 已有播放队列时去重追加歌曲，播放结束后自动切换下一首
